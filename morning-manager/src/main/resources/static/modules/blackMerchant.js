...
layui.define(['table', 'form'], function (exports) {
    ...
    table.render({
        elem: '#table-data'
        , method: 'post'
        , url: '/blackMerchant/list'
        , cols: [[
            {type: 'checkbox', fixed: 'left', align: 'center'}
            , {field: 'id', title: 'ID', align: 'center', minWidth: 100, hide: true}
            , {field: 'value', title: '参数', align: 'center', minWidth: 100}
            , {
                field: 'key', title: '类型', align: 'center', minWidth: 100, templet: function (d) {
                    var html = null;
                    if (d.key == '1') {
                        html = '<span class="layui-badge layui-bg-green">手机号</span>';
                    }
                    else if (d.key == '0') {
                        html = '<span class="layui-badge">身份证</span>';
                    }
                    return html;
                }
            }
            ...
            , {
                field: 'lastModTime', title: '最后一次修改时间', align: 'center', minWidth: 200, templet: function (d) {
                    if (d.lastModTime != null) {
                        return formatDate(d.lastModTime);
                    }
                }
            }
            , {field: 'lastUserId', title: '最后一次修改人', align: 'center', minWidth: 100}
        ...
        switch (obj.event) {
            case 'del':
                layer.confirm('确定删除吗?', {icon: 3, title: '提示'}, function (index) {
                    $.ajax({
                        type: "post"
                        , url: "/blackMerchant/delete/" + data.id
                        , dataType: "json"
                ...
                break;
            case 'edit':
                dialog('黑名单商户表修改', '/blackMerchant/editPage/' + data.id, '30%', '60%', 'edit-submit', function (index, field) {
                    $.ajax({
                        type: "post"
                        , url: "/blackMerchant/edit"
                        , data: field
                        , dataType: "json"
                        , success: function (result) {
                            if (result.code == 0) {
                ...
                break;
            case 'detail':
                dialogOk('黑名单商户表详情', '/blackMerchant/detailPage/' + data.id, '30%', '60%');
                break;
        }
    });
    //查询按钮
    ...
    var active = {
        ...
        //新增
        , add: function () {
            dialog('新增黑名单商户表', '/blackMerchant/addPage', '30%', '60%', 'add-submit', function (index, field) {
                $.ajax({
                    type: "post"
                    , url: "/blackMerchant/add"
                    , data: field
                    , dataType: "json"
                    , success: function (result) {
                        if (result.code == 0) {
