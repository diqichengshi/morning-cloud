...
layui.define(['table', 'form'], function (exports) {
    ...
    table.render({
        elem: '#table-data'
        , method: 'post'
        , url: '/riskBlackCard/list'
        , cols: [[
            {type: 'checkbox', fixed: 'left', align: 'center'}
            , {field: 'id', title: 'ID', align: 'center', width: 1, hide: true}
            , {field: 'cardNo', title: '银行卡号', align: 'center', minWidth: 200}
            , {
        ...
        switch (obj.event) {
            case 'del':
                layer.confirm('确定删除吗?', {icon: 3, title: '提示'}, function (index) {
                    $.ajax({
                        type: "post"
                        , url: "/riskBlackCard/delete/" + data.id
                        , dataType: "json"
                ...
                break;
            case 'edit':
                dialog('银行卡黑名单修改', '/riskBlackCard/editPage/' + data.id, '30%', '60%', 'edit-submit', function (index, field) {
                    $.ajax({
                        type: "post"
                        , url: "/riskBlackCard/edit"
                        , data: field
                        , dataType: "json"
                        , success: function (result) {
                            if (result.code == 0) {
                ...
                break;
            case 'detail':
                dialogOk('银行卡黑名单详情', '/riskBlackCard/detailPage/' + data.id, '30%', '60%');
                break;
        }
    });
    //查询按钮
    ...
    var active = {
        ...
        //新增
        , add: function () {
            dialog('新增银行卡黑名单', '/riskBlackCard/addPage', '50%', '60%', 'add-submit', function (index, field) {
                $.ajax({
                    type: "post"
                    , url: "/riskBlackCard/add"
                    , data: field
                    , dataType: "json"
                    , success: function (result) {
                        if (result.code == 0) {
